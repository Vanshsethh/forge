...
const statusFor = (agent, fleetPaused = false) => fleetPaused ? "revoked" : agent.status === "revoked" ? "revoked" : Number(agent.current_daily_spend) >= Number(agent.daily_cap) ? "capped" : "active";
...
export default function App() {
  ...
  const toggleFleet = async () => { await api.post(fleetKilled ? "/fleet/resume" : "/fleet/kill"); await refresh(); };
  ...
  const openIncidentCount = fleetKilled
    ? agents.length
    : agents.filter((agent) => agent.status === "revoked").length;
  const active = agents.filter(a => statusFor(a, fleetKilled) === "active").length;
  const displayStatus = (status) => status[0].toUpperCase() + status.slice(1);
  const filteredAgents = agents.filter((agent) => (statusFilter === "all" || statusFor(agent, fleetKilled) === statusFilter) && (typeFilter === "all" || agent.agent_type === typeFilter));

  const Fleet = () => <>
    <section className="stat-grid">
      <article className="glass-card"><div className="stat-label">Requests allowed today</div><div className="stat-value">{allowRate.toFixed(1)}%</div><div className="stat-copy">{fleetKilled ? `Fleet kill switch active — ${agents.length} agents revoked` : `Across ${active} active agents`}</div><div className="stat-bar"><span style={{ width: `${allowRate}%` }} /></div></article>
      ...
      <article className="glass-card"><div className="stat-label">Open incidents</div><div className="stat-value">{openIncidentCount}</div><div className="stat-copy">{fleetKilled ? `${openIncidentCount} agents blocked by fleet kill` : openIncidentCount ? `${openIncidentCount} ${openIncidentCount === 1 ? "requires" : "require"} operator review` : "No operator review required"}</div></article>
    </section>
